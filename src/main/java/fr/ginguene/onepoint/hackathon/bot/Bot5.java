	...
	public Response getResponse(Carte carte) {
		...
		for (Planete source : mesPlanetes) {
			...
			if (source.getPopulation() > 5) {
				...
				// Cas standards;
				/*
				 * Planete destination = null; int minScore = 0; for (Planete
				 * aPlanete : carte.getPlanetesEtrangeres()) {
				 * 
				 * int aScore = carte.getTrajetNbTour(source, aPlanete) * 2 +
				 * aPlanete.getPopulation() -
				 * carte.getMesFlottes(aPlanete.getId());
				 * 
				 * if (aScore < minScore || destination == null) { destination =
				 * aPlanete; minScore = aScore; }
				 * 
				 * }
				 * 
				...
				if (!mapCible.containsKey(source)) {
					...
					for (Planete aPlanete : carte.getPlanetesOrderByDistance(source)) {
						if (!mapCible.containsKey(aPlanete)) {
							...
							System.out.println("Select " + aPlanete + " for source " + source);
